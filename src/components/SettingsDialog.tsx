...
export default function SettingsDialog({ isOpen, onClose, courseName, dripConfig, schoolId, courseId, cohortId, onCopyCohortInviteLink }: SettingsDialogProps) {
    ...
    return (
        <div className="fixed inset-0 bg-white bg-opacity-70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div
                className="w-full max-w-md bg-[#f1f1f1] rounded-lg shadow-2xl relative"
                onClick={e => e.stopPropagation()}
            >
                {/* Dialog Header */}
                <div className="flex justify-between items-center p-6 pb-2">
                    <h2 className="text-xl font-light text-black">Settings</h2>
                    <button
                        onClick={onClose}
                ...
                <div className="flex flex-col gap-2 px-6 pt-4 pb-2">
                    <div className="flex items-center gap-2">
                        <span className="text-black text-base font-light">{courseId ? 'Course' : 'Cohort'}</span>
                        <div className="flex items-center gap-2 ml-auto">
                            <Tooltip content="Open" position="top">
                                <button
                                    ...
                                    className="text-gray-400 hover:text-black cursor-pointer flex items-center"
                    ...
                    <span className="text-black text-sm font-light">{courseName}</span>
                </div>

                {/* Release Schedule Section */}
                <div className="px-6 pb-6 pt-2">
                    <span className="text-black text-base font-light block mb-1">Course release schedule</span>
                    {dripConfig.is_drip_enabled ? (
                        <div className="space-y-1">
                            <span className="text-black text-sm font-light">{getFrequencyText()} </span>
                            <span className="text-black text-sm font-light">{`${dripConfig.publish_at ? `starting from ${formatDate(new Date(dripConfig.publish_at))}` : `after a learner joins the cohort`}`}</span>
                        </div>
                    ) : (
                        <div className="space-y-1">
                            <span className="text-black text-sm font-light">This course is not using a drip schedule. Learners can access all modules from the start.</span>
                        </div>
                    )}
                </div>
            </div>
